...
<div class="cards-1 cards-1--white">
    @if(session('message'))
...
<div class="pb-12">
    <form action="{{route('JobPostings.search')}}" method="POST">
        @csrf
    <div class="flex items-center max-w-md mx-auto  border border-indigo-600 bg-white rounded-lg ">
        ...
        <div class="flex flex-wrap items-center justify-center">
            @if(count($JobPostings) > 0)
                @foreach ($JobPostings->sortBy('status') as $JobPosting)
                <div class="bg-white font-semibold text-center rounded-3xl border shadow hover:shadow-xl p-10 w-full md:w-1/4 m-10 h-80 ">
                    ...
                    <h1 class="text-lg text-gray-700"> {{$JobPosting->name}} </h1>
                    <!-- <h3 class="text-sm text-gray-400 ">{{$JobPosting->id}} </h3> -->
                    <h3 class="text-sm text-gray-400 ">{{$JobPosting->currency}} </h3>

                    <h3 class="text-sm text-gray-400 ">{{$JobPosting->work_modality}} </h3>

                    <h3 class="text-sm text-gray-400 ">{{$JobPosting->location}} </h3>
            
                    <button  href="#details-lightbox-{{$JobPosting->id}}" style="cursor:pointer" class="transition-all bg-black-100 hover:bg-gray-600 popup-with-move-anim border-none outline-none focus:outline-none  px-8 py-2 mt-8 rounded-3xl text-white font-light uppercase tracking-wide">
                    @if( Session::get('lang') == 'es')  
                        Ver
                    @else
                        Open
            ...
            <!-- <table class="border-collapse table-auto w-full text-sm">
                ...
                <tbody class="bg-white dark:bg-slate-800">
                    @foreach ($JobPostings->sortBy('status') as $JobPosting)
                    <tr  href="#details-lightbox-{{$JobPosting->id}}" style="cursor:pointer" class="popup-with-move-anim">
	...
	<div id="details-lightbox-{{$JobPosting->id}}" class="lightbox-basic zoom-anim-dialog mfp-hide">
                    ...
                    <ul class="list-unstyled li-space-lg">
                       <li class="media">
                        ...
                        @else
                            Currency:
                        @endif
                            <span class="listing_highlight">{{$JobPosting->currency}}</span></div>
                      </li>
                      <li class="media">
                        <i class="fas fa-check"></i> <div class="media-body">
                        @if( Session::get('lang') == 'es')
                        ...
                        @else
                            Work modality:
                        @endif
                            <span class="listing_highlight">{{$JobPosting->work_modality}}</span></div>
                      </li>
                      <li class="media">
                        <i class="fas fa-check"></i> <div class="media-body">
                        @if( Session::get('lang') == 'es')
                        ...
                        @else
                            Hiring modality:
                        @endif
                            <span class="listing_highlight" >{{$JobPosting->hiring_modality}}</span></div>
                      </li>
                      <li class="media">
                        <i class="fas fa-check"></i> <div class="media-body">
                        @if( Session::get('lang') == 'es')
                        ...
                        @else
                            English level:
                        @endif
                            <span class="listing_highlight">{{$JobPosting->english_level}}</span></div>
                      </li>
        ...
        <div class="form-container mt-3">
            <form id="requestForm" data-toggle="validator" data-focus="false" action="{{route('JobPosting.apply')}}" method="POST" enctype="multipart/form-data">
                @csrf
