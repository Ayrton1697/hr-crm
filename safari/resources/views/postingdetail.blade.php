...
<table class="border-collapse table-auto w-full text-sm">
                ...
                <tbody class="bg-white dark:bg-slate-800">
                    ...
                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400">{{$job_posting->location}}</td>
                    ...
                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400">{{$job_posting->hiring_modality}}</td>
                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400">{{$job_posting->work_modality}}</td>
                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400">{{$job_posting->english_level}}</td>
                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400">{{$job_posting->currency}}</td>
                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400">{{$job_posting->seniority}}</td>
                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 text-slate-500 dark:text-slate-400">{{$job_posting->status}}</td>
                </tbody>
...
<div class="py-12">
        ...
        <!-- <form method="POST" action="{{ route('job_posting.edit_user_status') }}"> -->
        @csrf
        <script>
        ...
        @if(count($job_posting->applicants) == 0)
        ...
        @else
          ...
          @endif
            <table class="border-collapse table-auto w-full text-sm" id="applicants_table">
                ...
                <input type="hidden" id='job_posting_id' name='job_posting_id' value="{{$job_posting->id}}">
                <input type="hidden" id='app_url' name='job_posting_id' value="{{ENV('APP_URL')}}">
                  ...
                  <tr>
                    ...
                    <select class="posting_change search_status border-none focus:outline-none focus:border-none shadow-none font-medium text-slate-400 dark:text-slate-200 text-left bg-transparent" id="posting_change" name="posting_change" placeholder="Status" >
                        ...
                        @foreach($job_posting::All() as $job_posting_select)
                        <option class="select-option" value="{{$job_posting_select->id}}" id="{{$job_posting_select->id}}">{{mb_strimwidth($job_posting_select->name.', ' .$job_posting_select->company_name, 0, 40, "...") }}</option>
                        @endforeach
                  ...
                  @endforeach
                @else
                @foreach($job_posting->applicants as $applicant_posting)
                ...
                <input type="hidden" id='app_url' name='job_posting_id' value="{{ENV('APP_URL')}}">
                  ...
                  <tr>
                    ...
                    <select class="search_status border-none focus:outline-none focus:border-none shadow-none font-medium text-slate-400 dark:text-slate-200 text-left bg-transparent" id="status_search" name="status" placeholder="Status" >
                        ...
                        @foreach($job_posting::All() as $job_posting_select)
                        <option class="select-option" value="Activo" >{{mb_strimwidth($job_posting_select->name.', ' .$job_posting_select->company_name, 0, 40, "...") }}</option>
                        @endforeach
